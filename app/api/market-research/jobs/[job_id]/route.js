// Proxy for Market Research Job Status API
...
export async function GET(request, { params }) {
  try {
    ...
    const response = await fetch(
      `${BACKEND_API_URL}/market-research/jobs/${encodeURIComponent(job_id)}`,
      {
        method: 'GET',
        headers: {
    ...
    if (!response.ok) {
      ...
      console.error('[API Error] Backend job status API error:', {
    ...
    console.error('[API Error] Market Research job status proxy error:', {
